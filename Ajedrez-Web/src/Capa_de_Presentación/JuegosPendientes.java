package Capa_de_Presentación;

import java.io.IOException;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import Capa_de_Entidades.ControladorAjedrez;
import Capa_de_Entidades.Partida;

/**
 * Servlet implementation class JuegosPendientes
 */
@WebServlet("/JuegosPendientes")
public class JuegosPendientes extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public JuegosPendientes() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		int idPartida = Integer.parseInt(request.getParameter("idPartida"));
		ControladorAjedrez caj = new ControladorAjedrez();
		caj.buscarPartida(idPartida);
		Partida partidaActual = caj.getPartidaActual();
		
		request.getSession().setAttribute("partidaActual", partidaActual);
		
		request.getRequestDispatcher("JuegoActual.jsp").forward(request, response);
	}

}
